...
typedef void(AILCALL *AIL_QUICK_SET_VOLUME)(HAUDIO audio, float volume, float extravol);
...
typedef void(AILCALL *AIL_SET_STREAM_VOLUME)(HSTREAM stream, float volume);
...
void AILCALL AIL_quick_set_volume(HAUDIO audio, float volume, float extravol)
{
...
void AILCALL AIL_set_stream_volume(HSTREAM stream, float volume)
{
...
bool mss_compat_init()
{
    ...
    // Just on case one forget to release it.
    atexit(mss_compat_exit);
