...
// 0x60F038
static tig_button_handle_t tig_window_modal_dialog_button_handles[TIG_WINDOW_MODAL_BUTTON_MAX];

// 0x60F040
static int tig_message_modal_dialog_choice;
...
int tig_window_blit_art(tig_window_handle_t window_handle, TigArtBlitSpec* blit_spec)
{
    ...
    blit_spec->dst_video_buffer = win->video_buffer;

    rc = tig_art_blit(blit_spec);
    if (rc != TIG_OK) {
        return rc;
    }
...
int tig_window_get_at_position(int x, int y, tig_window_handle_t* window_handle_ptr)
{
    ...
    for (index = tig_window_num_windows - 1; index >= 0; index--) {
        window_handle = tig_window_stack[tig_window_num_windows];
        window_index = tig_window_handle_to_index(window_handle);
        win = &(windows[window_index]);
        if ((win->flags & TIG_WINDOW_HIDDEN) == 0
            && x >= win->frame.x
            ...
            && x <= win->frame.y + win->frame.height) {
            if ((win->flags & TIG_WINDOW_HAVE_TRANSPARENCY) != 0) {
            ...
            *window_handle_ptr = tig_window_stack[index];
...
bool tig_window_modal_dialog_create_buttons(int type, tig_window_handle_t window_handle)
{
    ...
    button_data2.field_20 = -1;
    button_data2.field_1C = -1;
    button_data2.field_24 = -1;
    button_data2.field_28 = -1;
    button_data2.flags = TIG_BUTTON_FLAG_0x01;
    button_data2.window_handle = window_handle;
    ...
    case TIG_WINDOW_MODAL_DIALOG_TYPE_0:
        ...
        tig_button_create(&button_data1, &(tig_window_modal_dialog_button_handles[0]));
        break;
    case TIG_WINDOW_MODAL_DIALOG_TYPE_1:
        ...
        tig_button_create(&button_data2, &(tig_window_modal_dialog_button_handles[1]));
        break;
    case TIG_WINDOW_MODAL_DIALOG_TYPE_2:
        // NOTE: Other cases do not check errors.
        if (tig_art_interface_id_create(823, 0, 0, 0, &(button_data1.art_id)) == TIG_OK
            && tig_art_frame_data(button_data1.art_id, &art_frame_data1) == TIG_OK
            && tig_art_interface_id_create(824, 0, 0, 0, &(button_data2.art_id)) == TIG_OK
            && tig_art_frame_data(button_data2.art_id, &art_frame_data2) == TIG_OK) {
            button_data1.x = 73;
            button_data1.y = 102;
            button_data1.width = art_frame_data1.width;
            button_data1.height = art_frame_data1.height;
            if (tig_button_create(&button_data1, &(tig_window_modal_dialog_button_handles[0])) != TIG_OK) {
                return false;
            }
            ...
            button_data2.width = art_frame_data2.width;
            button_data2.height = art_frame_data2.height;
            if (tig_button_create(&button_data2, &(tig_window_modal_dialog_button_handles[1])) != TIG_OK) {
                return false;
            }
        }
        break;
...
bool tig_window_modal_dialog_init()
{
    ...
    tig_window_modal_dialog_button_handles[1] = TIG_BUTTON_HANDLE_INVALID;
...
void tig_window_modal_dialog_exit()
{
    ...
    if (tig_window_modal_dialog_font != NULL) {
        tig_font_destroy(tig_window_modal_dialog_font);
        tig_window_modal_dialog_font = NULL;
    }

    for (index = 0; index < TIG_WINDOW_MODAL_BUTTON_MAX; index++) {
